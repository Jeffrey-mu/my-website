...
export default function App() {
      ...
      <>
        <div className={style.ToDoTab}>
          {TAB_BTN.map((item) => (
            <span
              key={item.name}
              style={item.value === toDoTab ? { color: '#2e8555' } : {}}
              onClick={() => {
  ...
  function addData() {
    return (
      showAdd && (
        <div className={clsx(style.dialog_mask)}>
  ...
  function Card() {
    ...
    const newCard = card.filter((c) =>
      toDoTab === 'c' ? !f(c.date) && c : f(c.date) && c
    )
